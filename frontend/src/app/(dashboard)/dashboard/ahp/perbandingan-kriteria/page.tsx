// File: frontend/src/app/(dashboard)/dashboard/ahp/perbandingan-kriteria/page.tsx
"use client";
...
import {
   ...
   FileText, PieChart, BookOpen, Activity,
   GitCommit, ChevronLeft, ChevronRight
} from "lucide-react";
...
const alurProsesAHP = [
   { label: "Input Matriks", desc: "Isi perbandingan berpasangan" },
   { label: "Normalisasi", desc: "Pembagian matriks kolom" },
   { label: "Vektor Prioritas", desc: "Perhitungan bobot (Eigen)" },
   { label: "Uji Konsistensi", desc: "Validasi & Kesimpulan AHP" },
];
   ...
   return (
      <div className="space-y-6 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
         <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border shadow-sm">
            ...
            {activeStep === 0 && (
               <Button onClick={handleSimpan} className="flex items-center gap-2 shadow-md px-6 bg-emerald-600 hover:bg-emerald-700" disabled={warnings.length > 0}>
         ...
         <div className="min-h-100">
            {isLoading ? (
               <div className="flex justify-center items-center py-20 text-slate-500"><RefreshCw className="animate-spin mr-2" size={24} /> Memuat data matriks...</div>
            ) : (
               <>
                           ...
                           <Table className="min-w-max border-collapse w-full">
                              <TableHeader>
                                 <TableRow className="bg-slate-100">
                                    ...
                                    <TableHead className="font-bold text-slate-700 border text-center" colSpan={kriteria.length}>1. Nilai Normalisasi Tiap Kriteria (X<sub>ij</sub>)</TableHead>
                                 ...
                                 <TableRow className="bg-slate-50">
                                    {kriteria.map(k => <TableHead key={k.kode} className="font-bold text-center text-slate-600 border text-xs">{k.kode}</TableHead>)}
                                 </TableRow>
                              </TableHeader>
                              <TableBody>
                                 {kriteria.map((baris) => {
                                 ...
                                 <TableRow className="bg-emerald-50/50">
                                    <TableCell colSpan={kriteria.length + 3} className="font-bold text-right border text-slate-600 text-xs pr-4">Total Penjumlahan Bobot =</TableCell>
                  ...
                  {/* === STEP 3: UJI KONSISTENSI & KESIMPULAN === */}
                        ...
                        <div className="border rounded-2xl bg-white shadow-sm p-6">
                           ...
                           <div className="mb-6 p-4 bg-purple-50/50 rounded-xl border border-purple-100 flex gap-3 text-sm text-purple-800">
                              ...
                              <div>
                                 ...
                                 <p className="mb-3 leading-relaxed text-slate-700">
                                    Nilai <strong>Weighted Sum Vector (WSV)</strong> didapatkan dengan menjumlahkan hasil perkalian antara nilai Matriks Awal dengan Bobot Prioritas (Eigenvector) pada setiap baris. Nilai WSV tersebut kemudian dibagi dengan bobot barisnya sendiri untuk mendapatkan <strong>Rasio</strong>.
                                 </p>
                           ...
                           <div className="overflow-x-auto rounded-xl border border-slate-200 mb-6">
                              <Table className="min-w-max border-collapse w-full">
                                 <TableHeader>
                                    <TableRow className="bg-slate-100">
                                       <TableHead className="font-bold text-slate-700 border text-center align-middle" rowSpan={2}>Kriteria</TableHead>
                                       <TableHead className="font-bold text-slate-700 border text-center" colSpan={kriteria.length}>1. Proses Perkalian: Matriks Awal × Eigenvector (W<sub>j</sub>)</TableHead>
                                       <TableHead className="font-bold text-purple-700 border text-center align-middle bg-purple-50 w-32" rowSpan={2}>2. Total Baris<br />(WSV)</TableHead>
                                       <TableHead className="font-bold text-emerald-700 border text-center align-middle bg-emerald-50 w-28" rowSpan={2}>Eigen Baris<br />(W<sub>i</sub>)</TableHead>
                                       <TableHead className="font-bold text-orange-700 border text-center align-middle bg-orange-50 w-32" rowSpan={2}>3. Rasio<br />(WSV / W<sub>i</sub>)</TableHead>
                                    </TableRow>
                                    <TableRow className="bg-slate-50">
                                       {kriteria.map(k => <TableHead key={k.kode} className="font-bold text-center text-slate-600 border text-xs">{k.kode}</TableHead>)}
                                    </TableRow>
                                 ...
                                 <TableBody>
                                    {kriteria.map((baris) => {
                                       ...
                                       // Menyimpan deret angka hasil perkalian untuk ditampilkan di sel WSV
                                       const rowProducts = kriteria.map(kolom => {
                                          const valAwal = matrix[baris.kode]?.[kolom.kode] || 1;
                                          const eigenKolom = ahpResult.bobot[kolom.kode] || 0;
                                       ...
                                       return (
                                          <TableRow key={`wsv-${baris.kode}`} className="hover:bg-slate-50">
                                             ...
                                             {kriteria.map((kolom) => {
                                                ...
                                                return (
                                                   <TableCell key={kolom.kode} className="text-center border text-slate-500 font-mono leading-tight align-middle">
                                                      <div className="font-bold text-slate-700 text-[12px]">{valAwal.toFixed(2)}</div>
                                                      <div className="text-[10px] mt-0.5 text-slate-400">× {eigenKolom.toFixed(4)}</div>
                                                   </TableCell>
                                                );
                                             })}

                                             {/* PERBAIKAN: Menampilkan angka penjumlahan kecil di bawah WSV */}
                                             ...
                                             <TableCell className="font-bold text-center border text-emerald-700 font-mono align-middle bg-emerald-50/30">
                                                {ahpResult.bobot[baris.kode]?.toFixed(4)}
                                             </TableCell>

                                             {/* PERBAIKAN: Menampilkan angka pembagian kecil di bawah Rasio */}
                                    ...
                                    <TableRow className="bg-slate-100">
                                       <TableCell colSpan={kriteria.length + 3} className="font-bold text-right border text-slate-700 pr-4">Rata-rata Rasio (λ<sub>max</sub>) =</TableCell>
